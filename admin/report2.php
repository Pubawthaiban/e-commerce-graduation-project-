...
<body>
		<h3 align="center">รายงาน สรุปยอดการขายตามช่วงวันที่กำหนด</h3>
        <div align="center">ระหว่างวันที่ <?=$_GET['date'];?> ถึง <?=$_GET['date1'];?></div>
...
<table border="1" cellpadding="9" cellspacing="9" align="center">
								...
								require('../connect.php');
								$con = "con";
                   $sql1 = mysql_query("select *,sum(detailnum) as num from db_order,db_orderdetail,product where db_order.status = '$con' and db_order.order_id = db_orderdetail.order_id and product.product_id = db_orderdetail.product_id and order_date between '$_GET[date]' and '$_GET[date1]' group by product_name")or die(mysql_error());
